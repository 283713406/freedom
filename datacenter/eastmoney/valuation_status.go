...
func (e EastMoney) QueryValuationStatus(ctx context.Context, secuCode string) (map[string]string, error) {
	...
	beginTime := time.Now()
	apiurl1, err := utils.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	logging.Debug(ctx, "EastMoney QueryValuationStatus "+apiurl1+" begin", zap.Any("params", params))
	if err != nil {
		return nil, err
	}
	resp := RespValuation{}
	if err := utils.HTTPGET(ctx, e.HTTPClient, apiurl1, nil, &resp); err != nil {
		return nil, err
	}
	...
	beginTime = time.Now()
	apiurl2, err := utils.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	logging.Debug(ctx, "EastMoney QueryValuationStatus "+apiurl2+" begin", zap.Any("params", params))
	if err != nil {
		return nil, err
	}
	if err := utils.HTTPGET(ctx, e.HTTPClient, apiurl2, nil, &resp); err != nil {
		return nil, err
	}
	latency = time.Now().Sub(beginTime).Milliseconds()
	logging.Debug(
	...
	beginTime = time.Now()
	apiurl3, err := utils.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	logging.Debug(ctx, "EastMoney QueryValuationStatus "+apiurl3+" begin", zap.Any("params", params))
	if err != nil {
		return nil, err
	}
	if err := utils.HTTPGET(ctx, e.HTTPClient, apiurl3, nil, &resp); err != nil {
		return nil, err
	}
	latency = time.Now().Sub(beginTime).Milliseconds()
	logging.Debug(
	...
	beginTime = time.Now()
	apiurl4, err := utils.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	logging.Debug(ctx, "EastMoney QueryValuationStatus "+apiurl4+" begin", zap.Any("params", params))
	if err != nil {
		return nil, err
	}
	err = utils.HTTPGET(ctx, e.HTTPClient, apiurl4, nil, &resp)
	latency = time.Now().Sub(beginTime).Milliseconds()
	logging.Debug(
		ctx,
