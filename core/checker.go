...
import (
	...
	"github.com/283713406/freedom/utils"
	"strings"
)
...
var DefaultCheckerOptions = CheckerOptions{
	...
	IsCheckEPSGrow:       false,
	IsCheckRevGrow:       false,
	IsCheckNetprofitGrow: false,
	MinGxl:               0.5,
}
...
func (c Checker) CheckFundamentals(ctx context.Context, stock models.Stock) (result CheckResult, ok bool) {
	...
	for _, rev := range revList {
		revs = append(revs, utils.YiWanString(rev))
	}
	...
	for _, np := range netprofitList {
		nps = append(nps, utils.YiWanString(np))
	}
	...
	if !utils.IsStrInSlice(stock.JZPG.GetValueTotalScore(), []string{"优秀", "良好"}) {
		ok = false
		itemOK = false
	}
	result[checkItemName] = map[string]string{
	...
	if len(stock.HistoricalFinaMainData) > 0 {
		...
		if !utils.IsStrInSlice(stock.GetOrgType(), []string{"银行", "保险"}) {
	...
	checkItemName = "市值"
	itemOK = true
	sz := utils.YiWanString(stock.BaseInfo.TotalMarketCap)
	...
	if c.Options.IsCheckMLLStability && !utils.IsStrInSlice(stock.GetOrgType(), []string{"银行", "保险"}) {
	...
	// 毛利率逐年递增 （只检测非金融股）
	if c.Options.IsCheckMLLGrow && !utils.IsStrInSlice(stock.GetOrgType(), []string{"银行", "保险"}) && len(mllList) > 0 {
	...
	if len(stock.HistoricalCashflowList) > 0 {
		desc = fmt.Sprintf(
			...
			utils.YiWanString(stock.NetcashOperate),
			utils.YiWanString(stock.NetcashInvest),
			utils.YiWanString(stock.NetcashFinance),
			utils.YiWanString(stock.NetcashFree),
		)
...
func getShillerPEValuation(f []float64, valuationCoefficient float64) string {
	...
	buyingPoints := utils.YiWanString(buyingPoint)
	sellingPoints := utils.YiWanString(sellingPoint)

	res := fmt.Sprintf("合理估值为%s * %f：按照当年合理估值7折买入，合理估值1.5倍卖出法，" +
		"可买入位置为%s，一年内卖点为%s。", utils.YiWanString(avg), valuationCoefficient, buyingPoints, sellingPoints)
