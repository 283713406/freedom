...
func (d ExportorData) GetHeaders() []string {
	return utils.StructTagList(&d, "csv")
}
...
func NewExportorData(ctx context.Context, stock Stock) ExportorData {
	...
	totalIncome5YList := stock.HistoricalFinaMainData.ValueList(ctx, eastmoney.ValueListTypeRevenue, 5, eastmoney.FinaReportTypeYear)
	tis := []string{}
	for _, ti := range totalIncome5YList {
		tis = append(tis, utils.YiWanString(ti))
	}

	netProfit5YList := stock.HistoricalFinaMainData.ValueList(ctx, eastmoney.ValueListTypeNetProfit, 5, eastmoney.FinaReportTypeYear)
	nps := []string{}
	for _, np := range netProfit5YList {
	...
	return ExportorData{
		...
		TotalIncome:     utils.YiWanString(fina.Totaloperatereve),
		TotalIncomeTBZZ: fina.Totaloperaterevetz,
		TotalIncome5Y: tis,

		NetProfit:     utils.YiWanString(fina.Parentnetprofit),
		...
		TotalMarketCap:         utils.YiWanString(stock.BaseInfo.TotalMarketCap),
		...
		NetcashOperate:      utils.YiWanString(stock.NetcashOperate),
		NetcashInvest:       utils.YiWanString(stock.NetcashInvest),
		NetcashFinance:      utils.YiWanString(stock.NetcashFinance),
		NetcashFree:         utils.YiWanString(stock.NetcashFree),
...
func (d ExportorDataList) GetIndustryList() []string {
	result := []string{}
	for _, stock := range d {
		if !utils.IsStrInSlice(stock.Industry, result) {
